...
async function exportToXLSX(answers) {
...
function normalizeToOption(txt, options) {
...
function buildUserPrompt(qPrompt, docText) {
...
function Labeled({ label, children }) {
  return (
    <label className="block text-sm font-medium text-gray-700 mb-1">{label}{children}</label>
  );
}
...
function TextArea({ value, onChange, rows=3, placeholder }) {
  return <textarea className="w-full border rounded-lg px-3 py-2" rows={rows} value={value || ''} onChange={e => onChange(e.target.value)} placeholder={placeholder}/>;
}

function Input({ value, onChange, placeholder, type="text" }) {
  return <input className="w-full border rounded-lg px-3 py-2" value={value || ''} onChange={e => onChange(e.target.value)} placeholder={placeholder} type={type}/>;
}
...
function Button({ children, onClick, variant="primary", disabled }) {
  ...
  return (
    <button disabled={disabled} onClick={onClick} className={classNames('px-3 py-2 rounded-lg text-sm font-medium shadow-sm disabled:opacity-50', styles)}>
      {children}
    </button>
  );
}
...
export default function App() {
  ...
  const [model, setModel] = useLocalStorage('qdrt:model', 'gpt-5');
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <header className="sticky top-0 z-10 bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <div className="text-xl font-semibold">QDRT – Interactive Reviewer</div>
          <div className="ml-auto flex items-center gap-2">
            <Button variant="outline" onClick={() => downloadBlob('qdrt_answers.json', new Blob([JSON.stringify(answers, null, 2)], {type:'application/json'}))}>Export JSON</Button>
            <Button variant="outline" onClick={() => exportToXLSX(answers)}>Export XLSX</Button>
            <Button onClick={generateAll} disabled={!apiKey || busy}>Generate All (AI)</Button>
          </div>
        </div>
      </header>
        ...
        <section className="lg:col-span-1 space-y-4">
          <div className="bg-white rounded-2xl shadow p-4">
            ...
            <div className="space-y-3">
              ...
              <input type="file" multiple accept=".pdf,.docx,.txt,.md" onChange={onUploadFiles} />
              <Labeled label="Extracted Text">
                <TextArea rows={10} value={docText} onChange={setDocText} placeholder="Paste or extract your SAD content here" />
              </Labeled>
            </div>
          </div>

          <div className="bg-white rounded-2xl shadow p-4">
            <h2 className="text-lg font-semibold mb-3">AI Settings</h2>
            <div className="space-y-3">
              <Labeled label="OpenAI API Key">
                <Input type="password" value={apiKey} onChange={setApiKey} placeholder="sk-..." />
              </Labeled>
              <Labeled label="Model">
                <Input value={model} onChange={setModel} placeholder="gpt-5" />
              </Labeled>
              <p className="text-xs text-gray-500">Your key is stored locally in your browser. The app calls OpenAI directly from your device.</p>
            </div>
          </div>
        </section>
        ...
        <section className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-2xl shadow p-4">
            <div className="flex items-center gap-3 mb-3">
              <Input value={filter} onChange={setFilter} placeholder="Search by # or text..." />
              <div className="ml-auto text-sm text-gray-500">{filteredQuestions.length} of {SCHEMA.questions.length} items</div>
            </div>
                ...
                return (
                  <div key={q.id} className="border rounded-xl p-4">
                    <div className="flex items-start gap-3">
                      <div className="text-sm font-semibold bg-gray-100 rounded-lg px-2 py-1">#{q.id}</div>
                      <div className="font-medium flex-1 whitespace-pre-line">{q.prompt}</div>
                      <div className="shrink-0 flex items-center gap-2">
                        <Button variant="outline" onClick={() => setAnswers(prev => ({...prev, [q.id]: {}}))}>Clear</Button>
                        <Button onClick={() => generateOne(q)} disabled={!apiKey || busy}>Generate</Button>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                      <div>
                        <Labeled label={SCHEMA.columns.C}>
                          <Select value={a.original_finding ?? q.defaults.original_finding} onChange={v => setAnswer(q.id, { original_finding: v })} options={SCHEMA.dropdowns.original_finding} />
                        </Labeled>
                      </div>
                      <div>
                        <Labeled label={SCHEMA.columns.D}>
                          <Select value={a.final_finding ?? q.defaults.final_finding} onChange={v => setAnswer(q.id, { final_finding: v })} options={SCHEMA.dropdowns.final_finding} />
                        </Labeled>
                      </div>
                      <div>
                        <Labeled label={SCHEMA.columns.F}>
                          <Select value={a.original_risk ?? q.defaults.original_risk} onChange={v => setAnswer(q.id, { original_risk: v })} options={SCHEMA.dropdowns.original_risk} />
                        </Labeled>
                      </div>
                      <div>
                        <Labeled label={SCHEMA.columns.G}>
                          <Select value={a.final_risk ?? q.defaults.final_risk} onChange={v => setAnswer(q.id, { final_risk: v })} options={SCHEMA.dropdowns.final_risk} />
                        </Labeled>
                      </div>
                      <div className="md:col-span-2">
                        <Labeled label={SCHEMA.columns.E}>
                          <TextArea rows={4} value={a.itemized_concerns ?? q.defaults.itemized_concerns} onChange={v => setAnswer(q.id, { itemized_concerns: v })} />
                        </Labeled>
                      </div>
                      <div className="md:col-span-2">
                        <Labeled label={SCHEMA.columns.H}>
                          <TextArea rows={3} value={a.comments ?? q.defaults.comments} onChange={v => setAnswer(q.id, { comments: v })} />
                        </Labeled>
                      </div>
                    </div>
                  </div>
