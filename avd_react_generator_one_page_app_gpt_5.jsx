...
/**
 * AVD React Generator – single‑file React component
 * -------------------------------------------------
 * What this does
 * - Lets you fill every major Architecture Vision Definition (AVD) section for a React app
 * - One "Generate" button per section (uses OpenAI API with model "gpt-5")
 * - "Generate all" orchestrates the lot
 * - "Download Word (.docx)" builds a clean document from your entries
 *
 * Security note
 * - Never hardcode secrets. Paste your API key at runtime. Optional localStorage remember toggle.
 */

// ---------- Minimal helpers ----------
const MODEL = "gpt-5"; // use ChatGPT‑5 via OpenAI API
...
// Major AVD sections derived from the uploaded template
// Keep ids stable; these are used for prompts and docx export
const SECTIONS = [
...
// AVD‑aware prompts for each section (React‑app focused). You can tweak tone/length live.
...
// ---------- OpenAI call (browser fetch variant) ----------
async function generateWithOpenAI({ apiKey, model, system, user }: { apiKey: string; model: string; system: string; user: string }) {
  ...
  // Responses API returns output in data.output[0].content[0].text (subject to SDK version).
  // Handle a couple of shapes defensively:
  const text =
...
// ---------- .docx export (on the fly import) ----------
async function exportToDocx(meta: Record<string, string>, sections: Record<string, string>) {
  const docx = await import("docx");
  const { Document, Packer, Paragraph, HeadingLevel, TextRun, AlignmentType, Table, TableRow, TableCell, WidthType } = docx as any;

  const para = (text: string) => new Paragraph({ children: [new TextRun(text)] });
  const h = (text: string, level: keyof typeof HeadingLevel = "HEADING_2") => new Paragraph({ text, heading: (HeadingLevel as any)[level] });
...
// ---------- UI ----------
export default function AVDReactGenerator() {
  ...
  function handleRememberToggle(next: boolean) {
    setRemember(next);
    if (next && apiKey) localStorage.setItem("avd_openai_key", apiKey);
    if (!next) localStorage.removeItem("avd_openai_key");
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-6 space-y-8">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">AVD React Generator</h1>
        <div className="text-sm opacity-70">Model: {MODEL}</div>
      </header>
      ...
      <section className="rounded-2xl border p-4 grid gap-3">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <input
            ...
            placeholder="Paste your OpenAI API key (kept in memory; optional localStorage)"
            value={apiKey}
            onChange={(e) => {
          ...
          <label className="inline-flex items-center gap-2 text-sm">
            <input type="checkbox" checked={remember} onChange={(e) => handleRememberToggle(e.target.checked)} />
      ...
      {/* Meta fields */}
      <section className="rounded-2xl border p-4 grid gap-4">
        <div className="font-medium">Document metadata</div>
        <div className="grid md:grid-cols-3 gap-3">
          {AVD_META_FIELDS.map((f) => (
            <div key={f.id} className="grid gap-1">
              ...
              <input
                ...
                value={meta[f.id] || ""}
                onChange={(e) => updateMeta(f.id, e.target.value)}
              />
            </div>
          ))}
        </div>
      ...
      {/* App summary */}
      <section className="rounded-2xl border p-4 grid gap-3">
        <div className="font-medium">App summary (high‑impact guidance for the generator)</div>
        <textarea
          className="min-h-[120px] w-full rounded-xl border p-3 font-mono"
          placeholder={"What problem the React app solves, target users, core flows, integrations, guardrails…"}
          value={appSummary}
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={handleGenerateAll} disabled={!!busy || !apiKey} className="rounded-2xl border px-4 py-2 hover:bg-gray-50 disabled:opacity-50">
          {busy ? "Generating…" : "Generate all sections"}
        </button>
        <button onClick={handleDownloadDocx} className="rounded-2xl border px-4 py-2 hover:bg-gray-50">Download Word (.docx)</button>
      </div>

      {/* Sections */}
      <section className="grid gap-6">
        {SECTIONS.map(({ id, title }) => (
          <div key={id} className="rounded-2xl border p-4 grid gap-2">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold">{title}</h2>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigator.clipboard.writeText(SECTION_DIRECTIONS[id] || "")}
                  className="rounded-xl border px-3 py-1 text-sm hover:bg-gray-50"
                  title="Copy section prompt"
                >Prompt</button>
                <button
                  onClick={() => handleGenerate(id)}
                  disabled={!!busy || !apiKey}
                  className="rounded-xl border px-3 py-1 hover:bg-gray-50 disabled:opacity-50"
                >{busy === id ? "Generating…" : "Generate"}</button>
              </div>
            </div>
            <textarea
              className="w-full min-h-[160px] rounded-xl border p-3 font-mono"
